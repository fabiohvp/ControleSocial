...
{
	public class StartsWith<T> : QueryableWorkflow<T>
			where T : class
	{
		...
		public StartsWith(string column, JArray args, bool not = false)
		{
			Column = column;
		...
		public override IQueryable<T> Execute(IQueryable<T> entryData)
		{
			...
			if (Not)
			{
				Func<string, Expression<Func<T, bool>>> expression = arg => row => !selector.Invoke(row).StartsWith(arg);
				return entryData
